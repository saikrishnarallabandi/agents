...
8. **Scoop check, both directions.** Search the project's own prior work first — restating your own
   earlier paper is the easiest miss. Then run the lit-reviewer custom agent (agent picker →
   lit-reviewer) for external prior art and fold its brief into the paper. If the claim
